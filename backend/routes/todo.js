    ...
    try {
        ...
        const newTodo = await prisma.todo.create({
            data: {
                ...
                userId: req.user.sub, // userId: req.user?.sub || "1", used for testing only without middleware
            },
        });
...
router.delete("/:todoId", async (req, res) => {
  ...
  try {
    // Find the todo to check if it exists
    const todo = await prisma.todo.findUnique({
      where: {
        id: todoId, // Match by ID
      },
    });
    ...
    // If the todo exists but is not completed, do not delete
    if (!todo.completed) {
      return res.status(400).json({
        success: false,
        message: "Todo must be completed before it can be deleted",
      });
    }
